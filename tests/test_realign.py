...
def test_realign_clean_drift(workspace_with_feature):
    ...
    result = realign(ws, "auth-flow")
    assert result["aligned"] is True
    assert result["repos"]["ui"]["status"] == "checkout_ok"
    assert result["repos"]["ui"]["before"] == "main"
    assert result["repos"]["ui"]["after"] == "auth-flow"
    # api wasn't drifted; should be already_aligned
    assert result["repos"]["api"]["status"] == "already_aligned"
...
def test_dirty_tree_raises_blocker(workspace_with_feature):
    ...
    # Drift + dirty in ui
    _git(["checkout", "main"], cwd=workspace_with_feature / "ui")
    (workspace_with_feature / "ui" / "src" / "App.tsx").write_text("modified\n")

    with pytest.raises(BlockerError) as exc_info:
        realign(ws, "auth-flow")
    err = exc_info.value
    assert err.code == "dirty_tree"
    assert "ui" in err.actual["dirty_repos"]
...
def test_auto_stash_succeeds_when_dirty(workspace_with_feature):
    ...
    ws = _make_workspace(workspace_with_feature)
    _git(["checkout", "main"], cwd=workspace_with_feature / "ui")
    (workspace_with_feature / "ui" / "src" / "App.tsx").write_text("dirty before realign\n")

    result = realign(ws, "auth-flow", auto_stash=True)
    assert result["aligned"] is True
    ui_entry = result["repos"]["ui"]
    ...
    assert "dirty before realign" not in (workspace_with_feature / "ui" / "src" / "App.tsx").read_text()
...
def test_branch_missing_reports_failed(workspace_with_feature):
    ...
    _features_file(workspace_with_feature, {
        "ghost-feature": {"repos": ["api", "ui"], "status": "active"},
    })
    ws = _make_workspace(workspace_with_feature)
...
def test_single_repo_feature_only_touches_its_repo(workspace_with_feature):
    ...
    ws = _make_workspace(workspace_with_feature)
    git.create_branch(workspace_with_feature / "ui", "ui-only")
    # ui starts on auth-flow (per fixture); needs to switch to ui-only
    _git(["checkout", "main"], cwd=workspace_with_feature / "api")  # api on main; should be ignored
...
def test_repos_override_filters_target(workspace_with_feature):
    ...
    ws = _make_workspace(workspace_with_feature)
    _git(["checkout", "main"], cwd=workspace_with_feature / "api")
    _git(["checkout", "main"], cwd=workspace_with_feature / "ui")

    result = realign(ws, "auth-flow", repos=["ui"])
    assert "api" not in result["repos"]
    assert result["repos"]["ui"]["status"] == "checkout_ok"
...
def test_unknown_repo_in_override_raises(workspace_with_feature):
    ...
    ws = _make_workspace(workspace_with_feature)
    with pytest.raises(BlockerError) as exc_info:
        realign(ws, "auth-flow", repos=["api", "ghost"])
